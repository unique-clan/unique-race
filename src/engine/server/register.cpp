...
void CRegister::RegisterUpdate(int Nettype)
{
	...
	if(m_RegisterState == REGISTERSTATE_START)
	{
		...
		m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "refreshing ip addresses");
	}
	else if(m_RegisterState == REGISTERSTATE_UPDATE_ADDRS)
	{
		...
		if(!m_pMasterServer->IsRefreshing())
		{
			...
			m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "fetching server counts");
			m_LastTokenRequest = Now;
	...
	else if(m_RegisterState == REGISTERSTATE_QUERY_COUNT)
	{
		...
		if(Left == 0 || Now > m_RegisterStateStart+Freq*5)
		{
			...
			{
				m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "WARNING: No master servers. Retrying in 60 seconds");
			...
			else
			{
				...
				m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", aBuf);
	...
	else if(m_RegisterState == REGISTERSTATE_HEARTBEAT)
	{
		...
		{
			m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "WARNING: Master server is not responding, switching master");
	...
	{
		if(m_RegisterFirst)
			m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "server registered");
...
int CRegister::RegisterProcessPacket(CNetChunk *pPacket, SECURITY_TOKEN ResponseToken)
{
	...
	{
		if(m_RegisterFirst)
			m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "no firewall/nat problems detected");
	...
	{
		m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_Sixup?"regsixup":"register", "ERROR: the master server reports that clients can not connect to this server.");
		char aBuf[256];
