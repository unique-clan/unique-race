...
bool CScoreWorker::ShowRank(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	if(!End)
	{
		...
		float Time = pSqlServer->GetFloat(2);
		int Format = pData->m_IsUnique ? TIME_HOURS_MILISECS : TIME_HOURS_CENTISECS;
		str_time_float(Time, Format, aBuf, sizeof(aBuf));
...
bool CScoreWorker::LoadMapType(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	const auto *pData = dynamic_cast<const CSqlLoadMapTypeRequest *>(pGameData);
	auto *pResult = dynamic_cast<CSqlLoadMapTypeResult *>(pGameData->m_pResult.get());

	// get the map type
	const char *pMapType = "SELECT Server FROM record_maps WHERE Map=?";
	if(!pSqlServer->PrepareStatement(pMapType, pError, ErrorSize))
	{
		return false;
	}
	pSqlServer->BindString(1, pData->m_aMap);
	...
	if(!End)
	{
		pSqlServer->GetString(1, pResult->m_aCurrentMapType, sizeof(pResult->m_aCurrentMapType));
	}
	return true;
}
