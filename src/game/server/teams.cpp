...
void CGameTeams::OnFinish(CPlayer *pPlayer, int TimeTicks, const char *pTimestamp, float FractionOfTick)
{
	...
	if(GameServer()->IsUniqueRace())
		Time = (FractionOfTick - GetStartTimeOffset(pPlayer) + TimeTicks) / (float)Server()->TickSpeed();
	...
	else if(Time < GameServer()->m_pController->m_CurrentRecord)
	{
		...
		if(g_Config.m_SvNamelessScore || !str_startswith(Server()->ClientName(ClientId), "nameless tee"))
		{
			...
			if(GameServer()->IsUniqueRace())
			{
				str_copy(GameServer()->m_pController->m_aCurrentRecordHolder, Server()->ClientName(ClientId));
