...
void CCharacter::SetWeapon(int W, bool Silent)
{
	...
	int Team = GameServer()->m_aTeamMapping[m_pPlayer->GetCid()];
	if(!Silent && (Team != -1 || GameServer()->IsUniqueRace()))
...
void CCharacter::SendDeathMessageIfNotInLockedTeam(int Killer, int Weapon, int ModeSpecial)
{
	if((Team() == TEAM_FLOCK || Teams()->TeamFlock(Team()) || Teams()->Count(Team()) == 1 || Teams()->GetTeamState(Team()) == ETeamState::OPEN || !Teams()->TeamLocked(Team())))
	{
		...
		{
			// only send death messages to players who show others
			if(pPlayer && Server()->ClientIngame(pPlayer->GetCid()) && (pPlayer->GetCid() == m_pPlayer->GetCid() || !GameServer()->IsUniqueRace() || pPlayer->m_ShowOthers != SHOW_OTHERS_OFF))
...
void CCharacter::DDRacePostCoreTick()
{
	...
	if(GameServer()->IsUniqueRace() && g_Config.m_SvFastcap && !Teams()->IsPractice(Team()))
	{
		// fastcap
		...
		if(d)
		{
			...
			for(int i = 0; i < End; i++)
			{
				...
				for(int CheckTeam = 0; CheckTeam <= TEAM_BLUE; ++CheckTeam)
				{
					...
					if(!m_aGotFastcapFlag[CheckTeam] && distance(Pos, GameServer()->m_pController->m_aFastcapFlag[CheckTeam].m_Pos) < 14 + GetProximityRadius())
					{
