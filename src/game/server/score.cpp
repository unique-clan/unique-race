...
void CScore::LoadMapType()
{
	if(m_pGameServer->m_pController->m_pLoadMapTypeResult)
		return; // already in progress

	auto LoadMapTypeResult = std::make_shared<CSqlLoadMapTypeResult>();
	m_pGameServer->m_pController->m_pLoadMapTypeResult = LoadMapTypeResult;

	auto Tmp = std::make_unique<CSqlLoadMapTypeRequest>(LoadMapTypeResult);
	str_copy(Tmp->m_aMap, Server()->GetMapName(), sizeof(Tmp->m_aMap));
	m_pPool->Execute(CScoreWorker::LoadMapType, std::move(Tmp), "load map type");
}
